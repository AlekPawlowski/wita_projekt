...
import { ESTATE_PARAM_ID, ESTATE_PARAM_NAME, MARGIN_SPACE } from "../../../config"
...
export const AddFailures = () => {
    ...
    const paramsEstateId = searchParams.get(ESTATE_PARAM_ID) as string;
    const paramsEstateName = searchParams.get(ESTATE_PARAM_NAME) as string;
    const [estateId, setEstateId] = useState<string | null>(null)
    const [estateData, setEstateData] = useState<IEstateFailData | null>(null);
    
    useEffect(()=>{
        if(paramsEstateId && paramsEstateName){
            setEstateData({
                id: paramsEstateId,
                name: paramsEstateName
            })
        }
        if(paramsEstateId){

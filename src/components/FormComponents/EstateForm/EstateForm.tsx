...
export const EstateForm = ({ formName, data }: IEstateForm) => {
    ...
    const {
        ...
        defaultValues: {
            ...
            owner_phone_number: isEditMode ? Number(data.owner_phone_number) : 0,
            revanue: isEditMode ? Number(data.revanue) : 0,
            electricity_amount: isEditMode ? Number(data.electricity_amount) : 0,
            electricity_deadline: isEditMode ? `${data.electricity_deadline}` : "",
            rent_amount: isEditMode ? Number(data.rent_amount) : 0,
            rent_deadline: isEditMode ? `${data.rent_deadline}` : "",
            tax_amount: isEditMode ? Number(data.tax_amount) : 0,
